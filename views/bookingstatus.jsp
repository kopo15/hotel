	...
	var events = [];		
		...
		<c:when test="${bookinglist.room1 ne '0'}">
			events.push({
				...
				url :'<c:url value='/admin/one?id=${bookinglist.id}'/>'
			});
		</c:when>
		...
		events.push({
			...
			url : '<c:url value='/admin/new?roomtype=1&in_date=${bookinglist.conf_checkindate} &out_date=${bookinglist.conf_checkoutdate}'/>'
	});
	</c:otherwise>
		</c:choose>
		...
		<c:when test="${bookinglist.room2 ne '0'}">
			events.push({
				...
				color:"#7fa59d",
				start : '<c:out value="${bookinglist.conf_checkindate}" />',
				end : '<c:out value="${bookinglist.conf_checkoutdate}" />', // Set end to the same date
				url : '<c:url value='/admin/one?id=${bookinglist.id}'/>'
		...
		<c:otherwise>
			...
			url : '<c:url value='/admin/new?roomtype=2&in_date=${bookinglist.conf_checkindate}&out_date=${bookinglist.conf_checkoutdate}'/>'
			});
		</c:otherwise>
		</c:choose>
		...
		<c:otherwise>
			...
			url : '<c:url value='/admin/new?roomtype=3&in_date=${bookinglist.conf_checkindate}&out_date=${bookinglist.conf_checkoutdate}'/>'
			});
		</c:otherwise>
		</c:choose>	
		</c:forEach>
